    ...
    Autodesk.ADN.Viewing.Extension.AddPartTool = function (viewer) {
        ...
        function AddPartTool() {
            ...
            this.activate = function () {
                ...
                //     var ex = viewer.getExtension("AddPartToolExtension")
                //     ex.tool.deactivate();
            ...
            function onSelectEvent(event) {
                ...
                if (_hitPoint) {
                    ...
                    else if (loadedModelUrn == "-") {
                        alertify.error("Please select model!");
                    }
                    else {
            ...
            function loadDocument(documentId, fragProxy) {
                Autodesk.Viewing.Document.load(documentId, (doc) => {
                    ...
                    viewer
                        ...
                        .then(function () { PlacementAgain(loadedModel); }
                        );
                });
            }

            // function setInstanceTree(model) {
            //     instanceTree = model.getData().instanceTree;
            //     console.log("set instanceTree");
            ...
            function PlacementAgain(model) {
                ...
                for (var i = 0; i < fraglist.fragments.fragId2dbId.length; i++) {
                    if (iTree != null) {
                        var parentId = fraglist.fragments.fragId2dbId[i];
                        var nodeName = iTree.getNodeName(parentId);
                    }
                    else if (iTree === null) {
                        //setInstanceTree(loadedModel);
                        iTree = model.getData().instanceTree;
                        console.log("test instanceTree");
                        console.log(iTree);
                        var parentId = fraglist.fragments.fragId2dbId[i];
                        var nodeName = iTree.getNodeName(parentId);
                    }
                ...
                alertify.success("Model loaded!");
                //console.log(tr);
                let loadedBox = model.getBoundingBox();
        ...
        _self.onToolbarCreated = function () {
            ...
            this._button = new Autodesk.Viewing.UI.Button('AddPartToolExtensionButtom');
            ...
            if (_self.toolactivated) {
                ...
                this._button.setToolTip('Add Model Extension');
            }
            this._button.addClass('addPartExtensionIcon');

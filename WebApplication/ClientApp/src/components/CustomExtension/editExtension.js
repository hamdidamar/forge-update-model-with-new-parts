    ...
    Autodesk.ADN.Viewing.Extension.EditModelTool = function (viewer) {
        ...
        function EditModelTool() {
            ...
            function onItemSelected() {
                ...
                if (_selectedModel != null) {
                    ...
                    if (_selectedModel != viewer.model) {
                        ...
                        alertify.error("Main model cannot edit!");
                        alertify.error("Enter a value within the wall boundaries.");
                        setInputsEmpty();
                    }
            ...
            function updateTxt(model) {
                var _selectedModelbBox = model.getBoundingBox();
            ...
            function setPosition(face, _selectedModel, fragbBox, _selectedModelbBox) {
                ...
                var error = false;
                if (face == "TopRef") {
                    var oldTopRef = Math.abs(fragbBox.max.y - _selectedModelbBox.max.y);
                    var oldBottomRef = Math.abs(fragbBox.min.y - _selectedModelbBox.min.y);
                    var totalValue = oldTopRef + oldBottomRef;
                    var newTopRef = document.getElementById(face).value;
                    ...
                    var oldTopRef = Math.abs(fragbBox.max.y - _selectedModelbBox.max.y);
                    var totalValue = oldTopRef + oldBottomRef;
                    var newBottomRef = document.getElementById(face).value;
                    ...
                    if (modelFace == "Front") {
                        var oldRightRef = Math.abs(fragbBox.max.x - _selectedModelbBox.max.x);
                        var oldLeftRef = Math.abs(fragbBox.min.x - _selectedModelbBox.min.x);
                        var totalValue = oldRightRef + oldLeftRef;
                        var newRightRef = document.getElementById(face).value;
                        ...
                        var oldRightRef = Math.abs(fragbBox.min.x - _selectedModelbBox.min.x);
                        var oldLeftRef = Math.abs(fragbBox.max.x - _selectedModelbBox.max.x);
                        var totalValue = oldRightRef + oldLeftRef;
                        var newRightRef = document.getElementById(face).value;
                        if (newRightRef <= totalValue + 1) {
                            var difference = newRightRef - oldRightRef;
                        ...
                        var oldRightRef = Math.abs(fragbBox.max.z - _selectedModelbBox.max.z);
                        var oldLeftRef = Math.abs(fragbBox.min.z - _selectedModelbBox.min.z);
                        var totalValue = oldRightRef + oldLeftRef;
                        var newRightRef = document.getElementById(face).value;
                        if (newRightRef <= totalValue + 1) {
                            var difference = newRightRef - oldRightRef;
                        ...
                        var oldRightRef = Math.abs(fragbBox.min.z - _selectedModelbBox.min.z);
                        var oldLeftRef = Math.abs(fragbBox.max.z - _selectedModelbBox.max.z);
                        var totalValue = oldRightRef + oldLeftRef;
                        var newRightRef = document.getElementById(face).value;
                        if (newRightRef <= totalValue + 1) {
                            var difference = newRightRef - oldRightRef;
                    ...
                    if (modelFace == "Back") {
                        ...
                        var oldRightRef = Math.abs(fragbBox.min.x - _selectedModelbBox.min.x);
                        var totalValue = oldRightRef + oldLeftRef;
                        var newLeftRef = document.getElementById(face).value;
            ...
            this.activate = function () {
                ...
                modelLoadContainer.style.minWidth = "215px";
                ...
                //     // set style
                //     var ex = document.getElementsByClassName("EditModelIcon");
                //     ex[0].style.backgroundColor = "blue";


                //     // var ex = document.getElementById("EditModelToolExtensionButton");
                //     // ex.click();
